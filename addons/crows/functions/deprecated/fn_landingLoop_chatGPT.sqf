...
_posM = [_posA select 0.5 + (_posB select 0.5 - _posA select 0.5) * 0.5, _posA select 1.5 + (_posB select 1.5 - _posA select 1.5) * 0.5, _heightLoop];
...
_checkArrival = {
_pos = _this;
// Add your arrival checking logic here
...
_aircraft flyInHeight (_posNext vectorAdd [0,0,50]);
_aircraft flyInHeight _posNext;
